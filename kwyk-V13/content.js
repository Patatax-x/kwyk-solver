...
(function() {
    ...
    async function checkRemoteConfig() {
        try {
            ...
            // Vérifier l'intégrité du code (anti-modification)
            if (remoteConfig.content_hash) {
                try {
                    const selfUrl = chrome.runtime.getURL('content.js');
                    const selfResponse = await fetch(selfUrl);
                    const selfCode = await selfResponse.text();
                    // SHA-256 du fichier
                    const hashBuffer = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(selfCode));
                    const localHash = Array.from(new Uint8Array(hashBuffer)).map(b => b.toString(16).padStart(2, '0')).join('');

                    if (localHash !== remoteConfig.content_hash) {
                        console.error('[Kwyk Tutor] ⛔ INTÉGRITÉ COMPROMISE — hash local:', localHash, 'attendu:', remoteConfig.content_hash);
                        extensionBlocked = true;
                        blockedMessage = 'Extension modifiée — Contactez l\'administrateur.';
                        return;
                    }
                    console.log('[Kwyk Tutor] ✓ Intégrité vérifiée');
                } catch (hashError) {
                    console.error('[Kwyk Tutor] Erreur vérification intégrité:', hashError);
                }
            }
    ...
    async function performInlineUpdate() {
        const config = window._kwykUpdateConfig;
        if (!config || !config.update_repo || !config.update_files) {
        ...
        try {
            ...
            for (const file of files) {
                ...
                if (!response.ok) {
                    console.error(`[Kwyk Tutor] Erreur téléchargement ${file}: HTTP ${response.status}`);
                    continue;
                }
            ...
            // Succès
            banner.innerHTML = `
                <span>v${config.version} installée ! Rechargez l'extension dans edge://extensions</span>
            `;
